...
#    choice(name: 'ENVIRONMENT', choices: ['DEV', 'QA', 'STG', 'PROD'], description: 'Environment to deploy in')
  ...
  stages {
    stage('Deploy') {
      steps {
        container('kompose') {
          withAWS(credentials: "carter-ecr"){
#            sh "kubectl config use-context ${params.ENVIRONMENT}"
            ...
            sh """
            TAG=${params.TAG} 
            if [ -e "${params.SERVICE}-k8s.yml" ]; then
              sed -e "s/\\\${TAG}/\$TAG/" "${params.SERVICE}-k8s.yml" > .deploy.yml
            else
              kompose convert -o .deploy.yml -f ${params.SERVICE}.yml
            fi
            """
